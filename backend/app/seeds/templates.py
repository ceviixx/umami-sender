from sqlalchemy.orm import Session
from app.database import SessionLocal
from app.models.template import MailTemplate

DEFAULT_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>Summary Report</title>
<style>
body {
margin: 0;
padding: 0;
background-color: #f1f1f1;
font-family: Arial, sans-serif;
}
.container {
max-width: 600px;
margin: 40px auto;
background-color: #ffffff;
padding: 30px;
border-radius: 10px;
}
.logo {
display: flex;
text-align: center;
margin-top: 30px;
margin-bottom: 10px;
align-items: center;
justify-content: center;
gap: 0.5rem;
}
.logo img {
max-width: 150px;
height: auto;
}
.logo h1 {
font-size: 24px;
color: rgb(37 99 235);
margin: 0;
}
h1 {
font-size: 22px;
color: #333333;
margin-bottom: 20px;
}
table {
width: 100%;
border-collapse: collapse;
margin-bottom: 30px;
}
th, td {
border: 1px solid #ddd;
padding: 10px;
text-align: left;
font-size: 14px;
}
th {
background-color: #f9f9f9;
font-weight: bold;
}
.section-title {
font-size: 18px;
margin: 20px 0 10px;
color: #333;
}
table.stats, 
table.stats th, 
table.stats td {
border: 0px solid black;
border-collapse: collapse;
background: none;
}
table th.value {
width: 150px;
}
</style>
</head>
<body>

<div class="logo">
<img src="data:image/png;base64," 
width="40" height="40" alt="UmamiSender Logo" />
<h1>UmamiSender</h1>
</div>

<div class="container">
<h1>Your summary for</h1>
<p>Report: {{ summary.name }}</p>
<p>Timerange: {{ summary.period }}</p>
<table class="stats">
	<tr>
		<th>Views</th>
		<th>Visits</th>
		<th>Visitors</th>
		<th>Bounce rate</th>
		<th>Visit duration</th>
	</tr>
	<tr>
		<td>{{ summary.stats.pageviews }}</td>
		<td>{{ summary.stats.visits }}</td>
		<td>{{ summary.stats.visitors }}</td>
		<td>{{ summary.stats.bounces }}</td>
		<td>{{ summary.stats.totaltime }}</td>
	</tr>
</table>

	<div class="section-title">Top Pageviews</div>
		<table>
			<tr>
				<th>Pages</th>
				<th class="value">Views</th>
			</tr>
			{% for entry in summary.pageviews %}
				<tr>
					<td>{{ entry.x }}</td>
					<td>{{ entry.y }}</td>
				</tr>
			{% endfor %}
		</tr>
	</table>
	
	<div class="section-title">Top Referrer</div>
	<table>
		<tr>
			<th>Referrer</th>
			<th class="value">Views</th>
		</tr>
		{% for entry in summary.referrers %}
			<tr>
				<td>{{ entry.x }}</td>
				<td>{{ entry.y }}</td>
			</tr>
		{% endfor %}
	</table>
	
</div>


<p style="text-align: center; font-size: 13px; color: #888888; margin-top: 40px;">
  Made with ❤️ by <a href="https://github.com/ceviixx/UmamiSender" style="color: #555555; text-decoration: none;">UmamiSender</a>
</p>
	


</body>
</html>"""

DISCORD_TEMPLTE = {
  "username": "UmamiSender",
  "avatar_url": "https://github.com/ceviixx/UmamiSender/blob/9b077046e4e35113f70591071d9447150536c6cb/frontend/public/umamisender.png",
  "content": "Latest statistics for: {{ summary.name }}",
  "embeds": [
    {
      "color": 1310975,
      "footer": {
        "text": "Timerange: {{ summary.period }}"
      },
      "fields": [
        {
          "name": "Views",
          "value": "{{ summary.stats.pageviews }}",
          "inline": True
        },
        {
          "name": "Visits",
          "value": "{{ summary.stats.visits }}",
          "inline": True
        },
        {
          "name": "Visitors",
          "value": "{{ summary.stats.visitors }}",
          "inline": True
        },
        {
          "name": "Bounce rate",
          "value": "{{ summary.stats.bounces }}",
          "inline": True
        },
        {
          "name": "Visit duration",
          "value": "{{ summary.stats.totaltime }}",
          "inline": True
        }
      ]
    },
    {
      "title": "Top Pageviews",
      "color": 65535,
      "footer": {
        "text": "Only the top 5"
      },
      "fields": [
        {
          "name": "Pages",
          "value": "{% for entry in summary.pageviews %}\n{{ entry.x }}\n{% endfor %}",
          "inline": True
        },
        {
          "name": "Views",
          "value": "{% for entry in summary.pageviews %}\n{{ entry.y }}\n{% endfor %}",
          "inline": True
        }
      ]
    },
    {
      "title": "Top Referrer",
      "color": 65535,
      "footer": {
        "text": "Only the top 5"
      },
      "fields": [
        {
          "name": "Pages",
          "value": "{% for entry in summary.referrers %}\n{{ entry.x }}\n{% endfor %}",
          "inline": True
        },
        {
          "name": "Views",
          "value": "{% for entry in summary.referrers %}\n{{ entry.y }}\n{% endfor %}",
          "inline": True
        }
      ]
    }
  ],
  "attachments": []
}

def seed_mail_templates():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="EMAIL", type="default").first():
        print("🌱 Seede Standard-Mail-Template...")
        template = MailTemplate(
            sender_type="EMAIL",
            description="Standard E-Mail-Report Template",
            html=DEFAULT_TEMPLATE
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'email.default' bereits vorhanden.")

    db.close()

def seed_discord_template():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="WEBHOOK_DISCORD", type="default").first():
        print("🌱 Seede Standard-Discord-Template...")
        template = MailTemplate(
            sender_type="WEBHOOK_DISCORD",
            description="Standard Discord Webhook Template",
            json=DISCORD_TEMPLTE
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'webhook.discord.default' bereits vorhanden.")

    db.close()
    
def seed_slack_template():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="WEBHOOK_SLACK", type="default").first():
        print("🌱 Seede Standard-Slack-Template...")
        template = MailTemplate(
            sender_type="WEBHOOK_SLACK",
            description="Standard Slack Webhook Template",
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'webhook.slack.default' bereits vorhanden.")

    db.close()

def seed_ms_teams_template():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="WEBHOOK_MS_TEAMS", type="default").first():
        print("🌱 Seede Standard-MS-Teams-Template...")
        template = MailTemplate(
            sender_type="WEBHOOK_MS_TEAMS",
            description="Standard MS-Teams Webhook Template",
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'webhook.ms_teams.default' bereits vorhanden.")

    db.close()

def seed_mattermost_template():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="WEBHOOK_MATTERMOST", type="default").first():
        print("🌱 Seede Standard-MS-Teams-Template...")
        template = MailTemplate(
            sender_type="WEBHOOK_MATTERMOST",
            description="Standard Mattermost Webhook Template",
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'webhook.mattermost.default' bereits vorhanden.")

    db.close()

def seed_custom_template():
    db: Session = SessionLocal()

    if not db.query(MailTemplate).filter_by(sender_type="WEBHOOK_CUSTOM", type="default").first():
        print("🌱 Seede Standard-Custom-Template...")
        template = MailTemplate(
            sender_type="WEBHOOK_CUSTOM",
            description="Standard Custom Webhook Template",
        )
        db.add(template)
        db.commit()
    else:
        print("✅ Template 'webhook.custom.default' bereits vorhanden.")

    db.close()

if __name__ == "__main__":
    seed_mail_templates()
    seed_discord_template()
    seed_slack_template()
    seed_ms_teams_template()
    seed_mattermost_template()
    seed_custom_template()
    